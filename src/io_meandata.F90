...
CASE ('benC_13   ')
    if (use_REcoM) then
      if (ciso) then
         call def_stream(nod2D,  myDim_nod2D,   'benC_13','Benthos Carbon-13','mmol', Benthos(:,5), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision,mesh)
      end if
    end if
CASE ('benC_14   ')
    if (use_REcoM) then
      if (ciso) then
        call def_stream(nod2D,  myDim_nod2D,   'benC_14','Benthos Carbon-14','mmol', Benthos(:,6), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision,mesh)
      end if
    end if
CASE ('benCalc_13')
    if (use_REcoM) then
      if (ciso) then
        call def_stream(nod2D,  myDim_nod2D,   'benCalc_13','Benthos calcite-13','mmol', Benthos(:,7), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision,mesh)
      end if
    end if
CASE ('benCalc_14')
    if (use_REcoM) then
      if (ciso) then
        call def_stream(nod2D,  myDim_nod2D,   'benCalc_14','Benthos calcite-14','mmol', Benthos(:,8), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision,mesh)
      end if
    end if
!ciso
CASE ('NPPn      ')
...
CASE ('LlimP     ')
    if (use_REcoM) then
    call def_stream(nod2D,  myDim_nod2D,   'LlimP','Nutrient limiting factor for Phy','[0-> 100& light limitation, 1->0%, 0-50m average]', diags2D(:,15), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
    end if

CASE ('LlimD     ')
    if (use_REcoM) then
    call def_stream(nod2D,  myDim_nod2D,   'LlimD','Nutrient limiting factor for Dia','[0-> 100& light limitation, 1->0%, 0-50m average]', diags2D(:,16), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
    end if
    
CASE ('NlimP     ')
    if (use_REcoM) then
    call def_stream(nod2D,  myDim_nod2D,   'NlimP','Light limiting factor for Phy','[0-> 100& light limitation, 1->0%, 0-50m average]', diags2D(:,17), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
    end if

CASE ('NlimD     ')
    if (use_REcoM) then
    call def_stream(nod2D,  myDim_nod2D,   'NlimD','Light limiting factor for Dia','[0-> 100& light limitation, 1->0%, 0-50m average]', diags2D(:,18), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
    end if
#endif
!___________________________________________________________________________________________________________________________________    
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   3D streams   <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
