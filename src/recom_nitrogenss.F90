...
!tracer_id(tr_num)==1013 !idian
        do nz=ul1,nl1
           nss(nz,n) = nss(nz,n) + (aux(nz))*dt/area(nz,n)/(zbar_3d_n(nz,n)-zbar_3d_n(nz+1,n)) ! we remove it drom idian
           LocDenit(n) = LocDenit(n) - (aux(nz))/area(nz,n)
        end do
